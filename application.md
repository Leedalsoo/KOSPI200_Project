폴더: Runtime Controller와 Use Case를 둔다.

[Child Page] use_cases
폴더 페이지

[Child Page] runtime_controller
폴더: Runtime Controller 계층.
[Child Page] runtime_controller.py
...
```
Controller는 Environment 구현체를 직접 import하지 않는다.

[Child Page] environment_hub
...
```
## 변경 원칙
    - background loop를 생성하지 않는다.
    - tick_entry는 one-shot injected tick entry로만 노출한다.
    - tick_entry가 존재하면 RuntimeTransportComposition도 필수다.
    - Router/OMS ownership 검증은 기존과 동일하다.
    - cancel_execution_receives()는 shutdown timeout 이후 blocked receive를 기술적으로 깨우는 capability이며 Domain 주문 상태를 변경하지 않는다.