...
from application.composition.live_runtime_composition_factory import build_live_bundle_from_components, create_live_runtime_controller
...
def test_live_runtime_controller_uses_explicit_live_builder():
    ...
    builder = build_live_bundle_from_components(market=components[0], broker=components[1], account=components[2], position=components[3], reconciler=components[4], recovery=components[5], safety_policy=LiveSafetyPolicy())
...
def test_live_runtime_builder_fails_closed_when_dependency_is_missing():
    with pytest.raises(ValueError, match="LIVE_RUNTIME_DEPENDENCY_REQUIRED:position"):
        build_live_bundle_from_components(market=FakeComponent(), broker=FakeBroker(), account=FakeComponent(), position=None, reconciler=FakeComponent(), recovery=FakeComponent(), safety_policy=LiveSafetyPolicy())
